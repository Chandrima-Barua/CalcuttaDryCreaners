...
<div class="container">
    <form method="POST" action="{{ route('orders.store') }}">
        @csrf
        <fieldset>
            <legend>{{ __('Create New B2C Order') }}</legend>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.addmore', function() {
        ...
        headstring =
            "<tr><th>Services</th><th>Delivery</th><th>Status</th><th>Packaging</th><th>Extra Charge</th><th>Price</th></tr>";
            ...
            success: function(response) {
                ...
                for (i = 0; i < rowdata['item'].length; i++) {
                    ...
                    thirdstring =
                        ...
                        "' name='packaging[]'></td><td align='left' class='dataextra'></td><td align='right' itemname=" +
                        rowdata['item'][i]['id'] + "  id='slug_" + rowdata['item'][i][
                            'slug'
                        ] +
                        "' class='unitprice' slug=" + rowdata['item'][i]['slug'] +
                        " style='color:white'>TK</td><input type='hidden' name='servicecharge[]' class='servicecharge' id='servicecharge_" +
                        rowdata['item'][i]['id'] +
    ...
    $(document).on('keyup', '.itemtable, .extrachage', function() {
        ...
        newcharge = parseInt($(this).val());
        prevcharge = parseInt($(this).siblings().val());
        if (Number.isNaN(prevcharge)) {
            prevcharge = 0;
        }
        if (Number.isNaN(newcharge)) {
            newcharge = 0;
        }
        charge = newcharge + prevcharge
        ...
        $(this).parent().siblings(".servicecharge").val(charge);
    ...
    $(document).on('change', '.urgency', function() {
        ...
        if ($(this).children("option:selected").hasClass("urgent") == true) {
            // console.log(Math.max.apply(Math, urgenttime))
            ...
            // console.log(Math.max.apply(Math, regulartime));
